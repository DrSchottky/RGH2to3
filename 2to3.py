...
with open(sys.argv[2], "rb") as f:
	fb = f.read()
cf_start = struct.unpack("!L", fb[0xc:0x10])[0]
fb_with_ecc = False
if cf_start != 0x0B0000:
    print("Unexpected CF start address. Aborting")
    sys.exit(0)
if len(fb) == 17301504:
    print("FB image contains spare data")
    patchable_fb = fb[:0xB5800]
    patchable_fb = ecc_utils.unecc(patchable_fb)
    fb_with_ecc = True
elif len(fb) == 50331648:
    print("FB image does not contain spare data")
    patchable_fb = fb[:0xB0000]
else:
...
print("I have to remove 0x{:02X} bytes after CE to make it fit. R.I.P. Xell, gone but not forgotten".format(new_size - original_size))
patchable_fb = patchable_fb[:original_size]

print("\nMerging image")
if fb_with_ecc:
    patchable_fb = ecc_utils.addecc(patchable_fb)
